...
for key in DATABASE_URL APP_ENV ALLOWED_RUNTIME_ENVIRONMENTS RELEASE_ENVIRONMENT ADMIN_AUTH_MODE \
  ...
  RAW_TEXT_LEGACY_KEKS_JSON API_KEY_SECRET_KEK_ID API_KEY_SECRET_KEK_BASE64 \
  API_KEY_SECRET_LEGACY_KEKS_JSON EMBEDDING_PROVIDER BGE_M3_MODEL_PATH \
...
apply_local_defaults() {
  ...
  export RELEASE_ENVIRONMENT="${RELEASE_ENVIRONMENT:-dev}"
  ...
  export API_KEY_SECRET_KEK_ID="${API_KEY_SECRET_KEK_ID:-local-api-key-secret-kek-001}"
  export API_KEY_SECRET_KEK_BASE64="${API_KEY_SECRET_KEK_BASE64:-AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=}"
  if [[ -z "${API_KEY_SECRET_LEGACY_KEKS_JSON:-}" ]]; then
