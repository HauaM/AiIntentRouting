...
def seed_pilot(
    *,
    ...
    threshold_preset = _threshold_preset_value(catalog)
    api: _AdminApi
    ...
    api.post(
        ...
        json={
            ...
            "default_threshold_preset": threshold_preset,
    ...
    for intent in catalog.intents:
        api.post(
            f"/admin/v1/services/{resolved_service_id}/intents",
            json=_intent_create_payload(intent),
        )
    for intent in catalog.intents:
        api.patch(
            f"/admin/v1/services/{resolved_service_id}/intents/{intent.intent_id}",
            json={"status": "active"},
        )
    for intent in catalog.intents:
        for text_raw in intent.positive_examples:
            example = api.post(
                f"/admin/v1/services/{resolved_service_id}/intents/{intent.intent_id}/examples",
                json=_example_payload("positive", text_raw),
            )
            api.patch(f"/admin/v1/services/{resolved_service_id}/examples/{example['example_id']}:approve")
    ...
    policy_version = api.post(
        f"/admin/v1/services/{resolved_service_id}/policy-versions",
        json={
    ...
    catalog_version = api.post(f"/admin/v1/services/{resolved_service_id}/catalog-versions")
    test_run = api.post(
        f"/admin/v1/services/{resolved_service_id}/test-runs",
        json={
    ...
    if threshold_preset == "balanced" and test_run["gate_passed"] is not True:
        raise RuntimeError(
    ...
    release = api.post(
        f"/admin/v1/services/{resolved_service_id}/releases",
        json={
            "environment": resolved_environment,
            "policy_version": policy_version["policy_version"],
            "intent_catalog_version": catalog_version["intent_catalog_version"],
    ...
    active_release = api.post(
        f"/admin/v1/services/{resolved_service_id}/releases/{release['release_version']}:activate"
    )
    ...
    return {
        ...
        "policy_version": policy_version["policy_version"],
        "intent_catalog_version": catalog_version["intent_catalog_version"],
        "test_runs": {threshold_preset: test_run},
