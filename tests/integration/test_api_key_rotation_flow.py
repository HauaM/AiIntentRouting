...
from scripts.rotate_api_key import rotate_api_key
...
def test_api_key_rotation_smokes_new_key_revokes_old_key_and_writes_audit_log(
    db_session: Session,
    ...
    audit_log = db_session.scalar(
        ...
        .where(models.AuditLog.event_type == "api_key.revoked")
        .where(models.AuditLog.target_id == old_state["key_id"])
    )
    assert audit_log is not None
    assert audit_log.actor_id == "api-key-rotation"
    assert audit_log.target_id == old_state["key_id"]
    assert out_state_path.exists()
    assert rotation["report"]["old_key_revoked"] is True
