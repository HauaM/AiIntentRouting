...
describe('runtime live test service', () => {
  ...
  it('calls the runtime endpoint with API-key auth and no Admin UI credentials', async () => {
  ...
  it('redacts the entered API Secret from runtime error fields before returning them', async () => {
    fetchMock.mockResolvedValue(
      new Response(
        JSON.stringify({
  ...
  it('redacts the entered API Secret from a network error before returning it', async () => {
