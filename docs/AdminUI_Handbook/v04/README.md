...
2. `examples/adminServices.ts`의 API 함수 패턴을 먼저 복사합니다.
3. `examples/AdminShell.tsx`와 `examples/ServiceScopeBar.tsx`로 기본 화면 골격을 잡습니다.
4. Phase 0 화면은 `IntentCatalogTable`, `RuntimeLogsTable`, `AuditLogsTable` 순서로 구현합니다.
5. Phase 1 쓰기 액션은 `ConfirmActionButton`을 통해서만 연결합니다.
6. Phase 2 기능은 `FutureFeatureNotice`로 비활성 처리하고 가짜 백엔드 상태를 만들지 않습니다.
...
- `Authorization: Bearer`를 사용하지 않습니다. 현재 Admin API는 custom header 모델입니다.
