...
2. `ONBOARDING_FLOW.md`에서 C-1/C-2/C-3 authorization-first onboarding 흐름을 확인합니다.
3. `E2E_DX_QA_CHECKLIST.md`에서 전체 UI E2E DX 리뷰 체크리스트를 확인합니다.
4. `SETUP_GUIDE.md`의 login/session cookie 설정을 먼저 적용합니다.
5. `examples/adminServices.ts`의 API 함수 패턴을 참고하되, 인증은 `withCredentials`와 서버 세션에 맡깁니다.
6. `examples/AdminShell.tsx`와 `examples/ServiceScopeBar.tsx`로 기본 화면 골격을 잡습니다.
7. Phase 0 화면은 `IntentCatalogTable`, `RuntimeLogsTable`, `AuditLogsTable` 순서로 구현합니다.
8. Phase 1 쓰기 액션은 서버에서 받은 service roles 기준으로 노출하고 `ConfirmActionButton`을 통해서만 연결합니다.
9. Phase 2 governed backend 계약은 구현됐지만, UI 라우트/API 연결과 UX 테스트가 추가될 때까지 버튼은 `FutureFeatureNotice` 또는 명시적 disabled 상태로 유지합니다.
...
| `ONBOARDING_FLOW.md` | C-1/C-2/C-3 authorization-first onboarding 기준 |
| `E2E_DX_QA_CHECKLIST.md` | QA 담당자용 전체 UI E2E DX 리뷰 체크리스트 |
...
- Service 등록, 권한 부여, API Key scope, runtime 연동, audit evidence는 온보딩 흐름의 일부로 다룹니다.
