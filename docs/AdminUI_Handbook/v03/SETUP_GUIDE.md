...
export default function access(initialState: { currentUser?: API.CurrentUser }) {
  ...
  return {
    ...
    // Phase 0: current API read access
    canViewDashboard:  has('system_admin') || has('service_operator'),
    canViewLogs:       has('system_admin') || has('service_operator') || has('auditor'),
    canViewAudit:      has('system_admin') || has('auditor'),
    canViewCatalog:    has('system_admin') || has('service_developer'),
    canViewReleases:   has('system_admin') || has('service_developer'),

    // Phase 1: current API write access
    canEditCatalog:     has('system_admin') || has('service_developer'),
    canApproveExample:  has('system_admin') || has('service_developer'),
    canCreateRelease:   has('system_admin'),
    canActivateRelease: has('system_admin'),
    canRollbackRelease: has('system_admin'),
    canManageKeys:      has('system_admin'),
    canViewRawQueryDirect: has('system_admin') || has('auditor'),

    // Phase 2 — 모두 false (future backend required)
    canViewRawQueryWithTwoPersonApproval: false,
...
| 기능 | Method | 경로 |
|---|---|---|
| Service 생성 | POST | `/admin/v1/services` |
| API Key 생성 | POST | `/admin/v1/api-keys` |
| API Key revoke | POST | `/admin/v1/api-keys/{key_id}:revoke` |
| Intent 목록 | GET | `/services/{sid}/intents` |
| Intent 상세 | - | 개별 GET 없음. 목록 응답의 선택 행 사용 |
...
| Example 생성 | POST | `/services/{sid}/intents/{intent_id}/examples` |
...
| Test Run 요약 | GET | `/services/{sid}/test-runs/{run_id}` |
| Test Run 결과 | GET | `/services/{sid}/test-runs/{run_id}/results` |
| Release 목록 | GET | `/services/{sid}/releases` |
| Active Release | GET | `/services/{sid}/releases/active?environment=prod` |
...
| Runtime Logs | GET | `/services/{sid}/runtime-logs?limit=100` |
| Runtime Log 상세 | GET | `/services/{sid}/runtime-logs/{trace_id}` |
| Raw Query audited decrypt | POST | `/services/{sid}/runtime-logs/{trace_id}:decrypt-raw-query` |
| Runtime Metrics | GET | `/services/{sid}/runtime-metrics?window_hours=24` |
| Audit Logs | GET | `/services/{sid}/audit-logs?limit=50&event_type=&trace_id=` |
...
export async function listIntents(serviceId: string, params: {
  current?: number; pageSize?: number; status?: string; route_key?: string; keyword?: string;
}) {
  const rows = await request<API.Intent[]>(base(serviceId), { method: 'GET' });
  const filtered = rows.filter((row) => {
    ...
    const matchesKeyword = !q || [row.intent_id, row.display_name, row.description]
...
export async function approveExample(serviceId: string, exampleId: string) {
  return request(`/services/${serviceId}/examples/${exampleId}:approve`, { method: 'PATCH' });
}

export async function fetchRuntimeMetrics(serviceId: string, windowHours = 24) {
  return request<API.RuntimeMetrics>(
    `/services/${serviceId}/runtime-metrics`,
    { method: 'GET', params: { window_hours: windowHours } },
  );
}

export async function fetchRuntimeLogs(serviceId: string, limit = 100) {
  return request<API.RuntimeLog[]>(
    `/services/${serviceId}/runtime-logs`,
...
<ProTable
  ...
  pagination={{ pageSize: 20 }} // 현재 서버 페이지네이션 없음. 클라이언트 페이지네이션.
/>
// 하단 고지 필수:
...
| support.js | 디자인 검토용 HTML에서 외부/누락 script 참조 금지. Umi 앱 구현 시 번들러 산출물만 배포 |
...
8. **Intent Catalog 조회** — ProTable 목록 + 선택 행 상세 패널

### Phase 1 (Write) — Phase 0 완성 후
9. Intent 생성/수정 ProForm
10. Example approve (현재 `PATCH ...:approve`, Modal.confirm)
11. Release 생성/activate/rollback (현재 system_admin 전용, Modal.confirm)
