# Handoff: Intent Routing Service — Admin Console (v1+ Design Proposal)

> **중요**: 이 문서는 **v1 이후 Admin Console의 설계 제안서**입니다.
> 현재 프로젝트는 **API-only MVP / Sprint 5 파일럿 리허설 단계**이며,
> Admin UI는 현재 구현 범위 밖입니다.
...
- 폐쇄망 배포 시 외부 CDN 참조(Pretendard 등)가 없어야 합니다 — `SETUP_GUIDE.md` §7 참조.
...
| Intent Catalog | intent 목록·상세 조회 | ✅ `/intents` GET |
...
| Examples | example 등록 / approve | ✅ POST + `:approve` |
...
| Releases | release 생성·activate·rollback | ✅ `:activate` / `:rollback` |
| API Keys | key 생성·revoke | ✅ (admin endpoint) |
...
| intent 생성/수정 | ✅ | 1 | |
| example 등록 | ✅ | 1 | |
| example approve | ✅ | 1 | `:approve` 액션 엔드포인트 |
| policy-version 생성 | ✅ | 1 | |
| catalog-version 생성 | ✅ | 1 | |
| test-run 생성/조회 | ✅ | 1 | |
| release 생성 | ✅ | 1 | |
| release activate | ✅ | 1 | `:activate` · Modal.confirm 필수 |
| release rollback | ✅ | 1 | `:rollback` · Modal.confirm 필수 |
...
- **API**: `GET /admin/v1/services/{service_id}/runtime-metrics?env=prod&range=24h`
- **레이아웃**: ProLayout 풀셸 (사이드바 178px + 헤더 44px + 본문 `#F4F6F8`)
- **상단 우측 필터**: env 선택(prod/stg) + 기간 선택(1h/24h/7d)
...
- **권한**: 전 역할 읽기 / service_developer 편집

### 3. Intent Catalog — 상세/편집 *(Phase 1)*
- **목적**: intent 속성 편집
- **API**: `GET/PATCH /admin/v1/services/{service_id}/intents/{intent_id}`
- **헤더 액션**: `임시저장` (PATCH draft) / `검토 요청` → Phase 2(🔮 pending 상태 미지원)
- **ProForm 필드**: intent_id(disabled) / route_key(Select) / 설명 / include keywords(mode=tags) / exclude keywords(mode=tags) / threshold_preset / risk_policy_override
- **우측 메타 카드**: 버전·embedding 상태·연결 release·수정자 (읽기 전용)
- **권한**: service_developer 편집
...
- **API**:
  ...
  - `POST /admin/v1/services/{service_id}/examples/{example_id}:approve`
- **Steps**: 등록 → 임베딩 생성 → 검토 → 승인
- **ProTable 컬럼**: 체크박스 / example 텍스트 / type(positive/negative Tag) / intent / embedding 상태 / 작업
- **일괄 approve**: `Modal.confirm` 으로 영향 범위 확인 후 처리
- **반려**: 🔮 Future backend required (사유 입력 엔드포인트 미확정)
- **권한**: service_developer 등록 / auditor approve
...
- **권한**: service_developer 생성 / system_admin·auditor activate

### 6. Runtime Logs *(Phase 0)*
- **목적**: API 호출 로그 조회 (query 기본 마스킹)
- **API**: `GET /admin/v1/services/{service_id}/runtime-logs?env=prod&range=1h`
- **필터**: 기간 DateRangePicker / service Select / decision 다중 Select / trace_id 검색
- **ProTable 컬럼**: time / trace_id(mono) / masked query(●) / route_key / decision Tag / latency(ms)
...
- **raw query 원문**: 🔮 Future backend required (2인 승인 + 시한부 토큰 엔드포인트 미확정)
- **live polling**: 🔮 Future backend required
- **권한**: service_operator 조회·필터 / 나머지 읽기
...
- **action 코드 예**: `release.activate` / `apikey.revoke` / `catalog.publish` / `example.approve`
- **상단 배너**: raw query 조회 승인 대기 — 🔮 Future backend required
- **하단 고지**: append-only · 보존 7년 · CSV export(🔮)
- **권한**: auditor 편집(승인) / 나머지 읽기
...
| `service_developer` | `isDeveloper` | Catalog CRUD, Examples, Releases 생성 |
| `service_operator` | `isOperator` | Runtime Logs 조회·필터 |
| `auditor` | `isAuditor` | Audit Logs 편집(승인), Example approve |
...
- 전역 `service_id` + `env` Context → 모든 API 요청에 반영
