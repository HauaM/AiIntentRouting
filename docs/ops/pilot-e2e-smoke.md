...
Use this Sprint 4 smoke before Dify handoff. It seeds a unique pilot service, checks `/healthz` and `/readyz`, runs the CSV threshold comparison with the required `balanced` gate, executes Dify-style runtime smokes, checks masked logs, and writes an e2e evidence index.
